...
def train_projection_heads(config: LayerwiseSemanticDynamicsConfig, 
    ...
    """
    Enhanced training with ALL LAYERS and comprehensive logging.
    ...
    logger.log("Starting enhanced training with ALL LAYERS...")
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch}/{config.epochs}"):
            ...
            with torch.no_grad():
                ...
                # ==============================================================
                # CHANGED: Use ALL LAYERS instead of just last layer
                # ==============================================================
                # Option 1: Weighted average (recommended)
                weights = torch.linspace(0.1, 1.0, hidden_states.shape[1]).to(DEVICE)  # Increasing weights
                weights = weights / weights.sum()  # Normalize
                all_layers_hidden = (hidden_states * weights.view(1, -1, 1)).sum(dim=1)
                
                # Option 2: Concatenate all layers (alternative)
                # all_layers_hidden = hidden_states.reshape(hidden_states.shape[0], -1)
                
                # Option 3: Mean across layers (simplest)
                # all_layers_hidden = hidden_states.mean(dim=1)
                # ==============================================================
                
                truth_embeddings = truth_encoder.encode_batch(truths)
            
            # Project to shared space and normalize
            hidden_projected = F.normalize(hidden_proj(all_layers_hidden), p=2, dim=-1)  # CHANGED: all_layers_hidden
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                weights = torch.linspace(0.1, 1.0, hidden_states.shape[1]).to(DEVICE)
                weights = weights / weights.sum()
                all_layers_hidden = (hidden_states * weights.view(1, -1, 1)).sum(dim=1)
                
                # Alternative: all_layers_hidden = hidden_states.reshape(hidden_states.shape[0], -1)
                # Alternative: all_layers_hidden = hidden_states.mean(dim=1)
                # ==============================================================
    ...
    logger.log("Enhanced training with ALL LAYERS completed successfully")
    return model_manager
# ==============================================================
